...
public final class Client {
   ...
   * only analyze.
   */
  public static final String DO_ANALYZE_PROPERTY = "doanalyze";
  public static final String ANALYZE_RANGE = "analyzerange";
  ...
  private static void exportMeasurements(Properties props, int opcount, long runtime)
    ...
    try {
      ...
      Measurements.getMeasurements().setThroughput(throughput);
  ...
  public static void main(String[] args) {
    ...
    boolean isGenReport = Boolean.valueOf(props.getProperty(GENERATE_REPORT, String.valueOf(false)));
    ...
    Measurements.setThreadcount(threadcount);

    boolean doAnalyze = Boolean.valueOf(props.getProperty(DO_ANALYZE_PROPERTY, String.valueOf(false)));
    if (doAnalyze){
      String cond = props.getProperty(ANALYZE_RANGE, "BY_WEEK");
      String currentDirectory = System.getProperty("user.dir");
      HtmlReporter reporter = new HtmlReporter(currentDirectory + "/report", Measurements.getDatabaseOperations());
      try {
        int jobId = Integer.parseInt(cond);
        reporter.generateReport();
      } catch(NumberFormatException e) {
        reporter.generateCompareHtml(cond);
      }
      return;
    }
    ...
    Measurements.setScenarioName(workload.getScenarioName());
  ...
  private static List<ClientThread> initDb(String dbname, Properties props, int threadcount,
    ...
    int maxExecutionTime = Integer.parseInt(props.getProperty(MAX_EXECUTION_TIME, "0"));
    ...
    try (final TraceScope span = tracer.newScope(CLIENT_INIT_SPAN)) {
      ...
      Measurements.saveTestConfig(dotransactions, opcount, maxExecutionTime);
  ...
  private static Properties parseArguments(String[] args) {
    ...
    while (args[argindex].startsWith("-")) {
      if (args[argindex].compareTo("-threads") == 0) {
        ...
        argindex++;
      } else if (args[argindex].compareTo("-analyze") == 0) {
        props.setProperty(DO_TRANSACTIONS_PROPERTY, String.valueOf(false));
        props.setProperty(DO_ANALYZE_PROPERTY, String.valueOf(true));
        argindex++;
